...
public class StashConnector implements Connector {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(StashConnector.class);
    ...
    public String getDiffByLine(String filename) throws URISyntaxException, IOException {
        URI uri = httpConnector.buildUri(createUrl(stashPatchset, DIFF_URL_FORMAT) + "/" + filename, new BasicNameValuePair("contextLines", "-1"), new BasicNameValuePair("srcPath", filename), new BasicNameValuePair("withComments", "true"));
        HttpGet httpGet = new HttpGet(uri);
        CloseableHttpResponse httpResponse = httpConnector.logAndExecute(httpGet);
        return httpConnector.consumeAndLogEntity(httpResponse);
    }
    ...
    private String createUrl(StashPatchset stashPatchset, String formatUrl) {
        return String.format(formatUrl, stashPatchset.getProjectKey(), stashPatchset.getRepositorySlug(), stashPatchset.getPullRequestId());
    }
    ...
    @java.beans.ConstructorProperties({"httpConnector", "stashPatchset"})
    @java.lang.SuppressWarnings("all")
    @javax.annotation.Generated("lombok")
    public StashConnector(final HttpConnector httpConnector, final StashPatchset stashPatchset) {
