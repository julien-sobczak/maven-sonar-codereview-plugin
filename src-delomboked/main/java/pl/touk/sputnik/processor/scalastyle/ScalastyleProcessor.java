...
public class ScalastyleProcessor implements ReviewProcessor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ScalastyleProcessor.class);
    ...
    private ReviewResult toReviewResult(List<Message> messages) {
        ...
        for (Message msg : messages) {
            ...
            if (msg instanceof StyleError) {
                ...
                reviewResult.add(new Violation(currentFileName, option(styleError.lineNumber(), -1), messageHelper.message(styleError.clazz().getClassLoader(), styleError.key(), styleError.args()), errorLevel(styleError.level())));
            }
        }
        return reviewResult;
    }
    ...
    @java.lang.SuppressWarnings("all")
    @javax.annotation.Generated("lombok")
    public ScalastyleProcessor(final Configuration config) {
        this.config = config;
    }
}